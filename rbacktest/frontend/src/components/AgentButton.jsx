...
export default function AgentButton({ onToggle }) {
  ...
  const dragging = useRef({ active: false, sx: 0, sy: 0, ox: 0, oy: 0, moved: false, frame: 0 });

  const onMouseDown = useCallback((e) => {
    if (e.button !== 0) return;
    const d = dragging.current;
    d.active = true; d.sx = e.clientX; d.sy = e.clientY;
    d.ox = pos.x; d.oy = pos.y; d.moved = false;
    e.preventDefault();
  }, [pos]);
  ...
  useEffect(() => {
    const onMove = (e) => {
      const d = dragging.current;
      if (!d.active) return;
      if (Math.abs(e.clientX - d.sx) > THRESHOLD || Math.abs(e.clientY - d.sy) > THRESHOLD) d.moved = true;
      if (!d.moved) return;
      if (d.frame) return;
      d.frame = requestAnimationFrame(() => {
        d.frame = 0;
        setPos({
          x: Math.max(8, Math.min(window.innerWidth - 64, d.ox + e.clientX - d.sx)),
          y: Math.max(60, Math.min(window.innerHeight - 64, d.oy + e.clientY - d.sy)),
        });
      });
    };
    const onUp = () => {
  ...
  return (
    <div ref={rootRef} className="agent-fab-root"
      style={{ right: pos.x, bottom: pos.y, left: "auto", top: "auto" }}>
      <button className="agent-fab-btn" aria-label="AI 助手" onMouseDown={onMouseDown}>
        <IconSparkle size={22} />
      </button>
    </div>
  );
