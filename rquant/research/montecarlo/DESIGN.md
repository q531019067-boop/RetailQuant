...
模拟 $N$ 条独立路径（$N = \text{simulations}$），每条路径走 $T = \text{forecast\_days}$ 步。

对每个时点 $t \in \{0, 1, \ldots, T\}$：
- 5 分位 $P_{05}(t) = \text{percentile}_{5\%}(\{S^{(i)}_t\}_{i=1}^{N})$
...
$$
\hat{\mu}_{\text{annual}} = \hat{\mu} \times 252, \quad \hat{\sigma}_{\text{annual}} = \hat{\sigma} \times \sqrt{252}
$$
...
$$
\text{input}(\text{forecast}) = \text{K 线}(\text{date} \leq D_{\text{now}}) + \text{current\_price}(\text{at } D_{\text{now}})
$$
