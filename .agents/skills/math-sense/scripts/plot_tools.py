...
def op_histogram(data, bins=20, output=None, title='Histogram', xlabel='Value', ylabel='Frequency'):
    ...
    plt.close(fig)
    return {
        "image": output, "bins": bins,
        "mean": mean, "std": float(np.std(arr)), "count": len(arr),
    }
...
OPERATIONS = {
    'curve': op_curve, 'histogram': op_histogram, 'scatter': op_scatter,
    'comparison': op_comparison, 'polar': op_polar, 'ascii': op_ascii_plot,
}
