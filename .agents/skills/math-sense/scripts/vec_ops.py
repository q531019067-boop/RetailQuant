...
def _quat_to_euler(q, order="ZYX"):
    ...
    return {
        ...
        "warning": "pitch≈±90°时出现万向节锁——yaw和roll不可区分"
        if abs(abs((2 * (w * y - z * x) if order == "ZYX" else 0)) - 1) < 1e-6
        else "",
    }
...
OPERATIONS = {
    ...
    "lerp": op_lerp,
    "slerp": op_slerp,
    "rotate2d": op_rotate2d,
    "rotate3d": op_rotate3d,
    "mat_mul": op_mat_mul,
    "mat_det": op_mat_det,
    "mat_inv": op_mat_inv,
    "mat_transpose": op_mat_transpose,
    "mat_eigen": op_mat_eigen,
    "solve_linear": op_solve_linear,
    "quat_from_axis_angle": op_quat_from_axis_angle,
    "quat_mul": op_quat_mul,
    "quat_rotate": op_quat_rotate,
    "quat_slerp": op_quat_slerp,
    "euler_to_quat": op_euler_to_quat,
    "quat_to_euler": op_quat_to_euler,
    "euler_to_matrix": op_euler_to_matrix,
    "matrix_to_quat": op_matrix_to_quat,
}
