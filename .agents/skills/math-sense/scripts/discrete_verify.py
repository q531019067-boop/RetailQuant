...
def op_check(expr_str, range_, n, properties):
    ...
    for prop in properties:
        ...
        if '极值点个数=' in prop:
            ...
            mid = (x_min + x_max) / 2
            left = ys[xs < mid]; right = ys[xs >= mid][::-1]
...
def _fit_polynomial_iterative(xs, ys, max_degree=8):
    ...
    return {
        "method": "polynomial", "best_degree": best_deg,
...
def _fit_fourier_iterative(xs, ys):
    ...
    return {
        "method": "fourier", "best_harmonics": best_n,
        "a0": float(best_coeffs[0]),
        "coefficients": [{"k": k, "a": float(best_coeffs[2*k-1]), "b": float(best_coeffs[2*k])}
...
def _fit_exponential_iterative(xs, ys):
    ...
    r2 = 1 - ss_res / ss_tot if ss_tot > 0 else 0
    return {
        "method": "exponential", "formula": f"{a:.4g}*exp({b:.4g}*x)",
        "a": float(a), "b": float(b), "r_squared": float(r2),
    }
...
OPERATIONS = {
    'check': op_check, 'singularities': op_singularities,
