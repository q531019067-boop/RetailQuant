...
def op_describe(data):
    ...
    if HAS_NUMPY:
        ...
        return {"count": n, "mean": mean, "std": math.sqrt(var), "var": var,
...
def op_cdf_build(items):
    ...
    for item in items:
        cum += item['weight']
        cdf.append({"value": item['value'], "weight": item['weight'],
                    "prob": item['weight'] / total, "cum_prob": cum / total})
    ...
    for _ in range(n_samples):
        ...
        for item in cdf:
            if r <= item['cum_prob']:
                ...
                break
    return {"samples": results, "n": n_samples,
            "probs": {k: v / n_samples for k, v in results.items()}}
...
def op_monte_carlo(expr, distributions, n_samples=10000, seed=None):
    ...
    for _ in range(n_samples):
        ...
        ALLOWED = {'sin': math.sin, 'cos': math.cos, 'exp': math.exp, 'log': math.log,
                   'sqrt': math.sqrt, 'abs': abs, 'max': max, 'min': min, 'pi': math.pi, 'e': math.e}
        try:
...
def op_prd(nominal_pct, n_trials=1000, seed=None, constant=None):
    ...
    import random as _r
    if seed is not None: _r.seed(seed)
    nominal_p = nominal_pct / 100.0
    if constant is None: constant = _find_prd_C(nominal_p)
    else: constant = constant / 100.0
    triggers, streaks, current_p, streak = 0, [], constant, 0
    for _ in range(n_trials):
        streak += 1
        if _r.random() < current_p:
            triggers += 1; streaks.append(streak); streak = 0; current_p = constant
        else:
    ...
    return {"nominal_pct":nominal_pct,"C_pct":round(constant*100,4),
            "actual_pct":round(triggers/n_trials*100,2),"avg_streak":round(avg_s,2),
            "prd_var":round(prd_var,2),"true_var":round(true_var,2),
...
def _find_prd_C(P_target, max_iter=200):
    ...
    for _ in range(max_iter):
        ...
        if abs(ep - P_target) < 0.0002: break
        C += (P_target - ep) * 0.15
    return max(C, 0.0001)
...
def op_pity(base_rate, pity_count, boost_after=0, boost_amount=0, n_trials=100000, seed=None):
    ...
    import random as _r
    if seed is not None: _r.seed(seed)
    bp = base_rate / 100.0; total, pity_hits, max_streak = 0, 0, 0
    streak = 0; current_p = bp
    for _ in range(n_trials):
        streak += 1
        if streak >= boost_after > 0:
            current_p = min(1.0, bp + (streak - boost_after + 1) * boost_amount / 100.0)
        if streak >= pity_count:
            total += 1; pity_hits += 1; streak = 0; current_p = bp
        elif _r.random() < current_p:
            total += 1; streak = 0; current_p = bp
        max_streak = max(max_streak, streak)
    actual_rate = total / n_trials * 100
    expected_cost = n_trials / total if total > 0 else float('inf')
    return {"base_rate_pct": base_rate, "pity_count": pity_count,
            "actual_rate_pct": round(actual_rate, 2), "expected_draws": round(expected_cost, 1),
...
def op_sensitivity(expr, vars_dict, perturb=0.1):
    ...
    ALLOWED = {'sin':_m.sin,'cos':_m.cos,'tan':_m.tan,'exp':_m.exp,'log':_m.log,
               'sqrt':_m.sqrt,'abs':abs,'pow':pow,'pi':_m.pi,'e':_m.e,'max':max,'min':min}
    tree = _ast.parse(expr.strip(), mode='eval')
    def _ev(n, vd):
        if isinstance(n, _ast.Constant): return n.value
        if isinstance(n, _ast.Name):
            if n.id in vd: return vd[n.id]
        ...
        if isinstance(n, _ast.UnaryOp):
            v=_ev(n.operand,vd); return -v if isinstance(n.op,_ast.USub) else +v
        if isinstance(n, _ast.Call):
            fn=_ev(n.func,vd); return fn(*[_ev(a,vd) for a in n.args])
        return 0
    base = _ev(tree.body, vars_dict)
    results = []
    for var, val in vars_dict.items():
        vd_up = dict(vars_dict); vd_up[var] = val * (1 + perturb)
        vd_down = dict(vars_dict); vd_down[var] = val * (1 - perturb)
        up = _ev(tree.body, vd_up); down = _ev(tree.body, vd_down)
        elasticity = (up - down) / (2 * val * perturb) * val / base if base > 0 else 0
        results.append({"var": var, "base_val": val, "abs_change_per_pct": round((up-down)/(2*perturb),4),
                        "elasticity": round(elasticity, 4),
                        "direction": "increase" if up>down else "decrease"})
    results.sort(key=lambda x: abs(x['elasticity']), reverse=True)
    return {"base_result": base, "perturbation_pct": perturb*100, "ranked": results,
...
def op_prd_inverse(target_interval, max_iter=200):
    ...
    return {"target_interval": target_interval, "nominal_rate_pct": round(P_target*100,2),
