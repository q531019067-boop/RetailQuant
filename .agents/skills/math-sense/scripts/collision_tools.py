...
def _v(s):
    if isinstance(s, (list, tuple)): return np.array([float(v) for v in s[:3]])
    if isinstance(s, str):
        s = s.strip()
        if s.startswith('['): return np.array([float(v) for v in json.loads(s)[:3]])
...
def op_sphere_contains_point(center, radius, point):
    ...
    return {"contains": d <= r, "distance": float(d), "penetration": float(r - d) if d <= r else 0,
            "center": c.tolist(), "radius": r, "point": p.tolist()}

def op_sphere_sphere(c1, r1, c2, r2):
    c1, c2 = _v(c1), _v(c2); r1, r2 = float(r1), float(r2)
    d = np.linalg.norm(c2 - c1)
    intersect = d <= r1 + r2
    return {"intersect": intersect, "distance_between_centers": float(d),
            "r1": r1, "r2": r2, "sum_radii": r1 + r2,
...
def op_ray_sphere(ray_origin, ray_dir, center, radius):
    ...
    pts = []
    if t1 >= 0: pts.append({"t": float(t1), "point": (o + t1 * d_vec).tolist()})
    if t2 >= 0 and abs(t2 - t1) > 1e-8: pts.append({"t": float(t2), "point": (o + t2 * d_vec).tolist()})
    return {"hit": len(pts) > 0, "t_entry": float(t1) if t1 >= 0 else None,
            "t_exit": float(t2) if t2 >= 0 else None, "points": pts}

def op_sphere_aabb(center, radius, aabb_min, aabb_max):
    c, r = _v(center), float(radius); mn, mx = _v(aabb_min), _v(aabb_max)
    closest = np.clip(c, mn, mx)
    d = np.linalg.norm(closest - c)
    return {"intersect": d <= r, "closest_point_on_aabb": closest.tolist(),
            "distance": float(d), "penetration": float(r - d) if d <= r else 0}

def op_sphere_triangle(center, radius, v0, v1, v2):
    c, r = _v(center), float(radius); v0,v1,v2 = _v(v0),_v(v1),_v(v2)
    closest = _closest_point_on_triangle(v0, v1, v2, c)
    d = np.linalg.norm(closest - c)
    return {"intersect": d <= r, "closest_point": closest.tolist(),
            "distance": float(d), "penetration": float(r - d) if d <= r else 0}
...
def op_capsule_point(cap_a, cap_b, cap_r, point):
    a, b, p = _v(cap_a), _v(cap_b), _v(point); r = float(cap_r)
    closest, t = _closest_point_on_segment(a, b, p)
    d = np.linalg.norm(p - closest)
    return {"contains": d <= r, "distance": float(d), "closest_on_axis": closest.tolist(),
            "axis_param_t": float(t), "penetration": float(r - d) if d <= r else 0}

def op_capsule_sphere(cap_a, cap_b, cap_r, sph_c, sph_r):
    a, b, sc = _v(cap_a), _v(cap_b), _v(sph_c); cr, sr = float(cap_r), float(sph_r)
    closest, _ = _closest_point_on_segment(a, b, sc)
    d = np.linalg.norm(sc - closest)
    return {"intersect": d <= cr + sr, "distance": float(d), "sum_radii": cr + sr,
            "penetration": float(cr + sr - d) if d <= cr + sr else 0}

def op_capsule_capsule(a1, b1, r1, a2, b2, r2):
    a1,b1,a2,b2 = _v(a1),_v(b1),_v(a2),_v(b2); r1,r2 = float(r1),float(r2)
    # 线段到线段最短距离
    def _seg_seg_dist(A, B, C, D):
        AB, CD = B-A, D-C; AC = C-A
        d00 = np.dot(AB,AB); d11 = np.dot(CD,CD)
        if d00 < 1e-15: return np.linalg.norm(_closest_point_on_segment(C,D,A)[0]-A)
        if d11 < 1e-15: return np.linalg.norm(_closest_point_on_segment(A,B,C)[0]-C)
        d01 = np.dot(AB,CD); d10 = d01
        d20 = np.dot(AC,AB); d21 = np.dot(AC,CD)
        denom = d00*d11 - d01*d10
        if abs(denom) < 1e-15:
            t = 0; s = np.clip(d20/d00, 0, 1)
        else:
    ...
    return {"intersect": d <= r1 + r2, "segment_distance": float(d),
            "sum_radii": r1 + r2, "penetration": float(r1 + r2 - d) if d <= r1 + r2 else 0}

def op_capsule_aabb(cap_a, cap_b, cap_r, aabb_min, aabb_max):
    a, b, mn, mx = _v(cap_a), _v(cap_b), _v(aabb_min), _v(aabb_max); r = float(cap_r)
    # 采样胶囊轴上的点，找最近的AABB面
    n_samples = 20
    min_dist = float('inf')
    ...
    return {"intersect": min_dist <= r, "min_distance": float(min_dist),
...
def op_aabb_contains_point(aabb_min, aabb_max, point):
    ...
    return {"contains": inside, "closest_point": closest.tolist(),
...
def op_ray_aabb(ray_origin, ray_dir, aabb_min, aabb_max):
    o, d_vec = _v(ray_origin), _v(ray_dir); mn, mx = _v(aabb_min), _v(aabb_max)
    d_vec = d_vec / max(np.linalg.norm(d_vec), 1e-15)
    t_min = float('-inf'); t_max = float('inf')
    for i in range(3):
        if abs(d_vec[i]) < 1e-15:
            if o[i] < mn[i] or o[i] > mx[i]: return {"hit": False}
        else:
            t1 = (mn[i] - o[i]) / d_vec[i]
            t2 = (mx[i] - o[i]) / d_vec[i]
    ...
    return {"hit": t_max >= 0, "t_entry": float(t_min), "t_exit": float(t_max),
            "entry_point": p_entry}
...
def op_triangle_point(v0, v1, v2, point):
    v0,v1,v2 = _v(v0),_v(v1),_v(v2); p = _v(point)
...
def op_triangle_ray(v0, v1, v2, ray_origin, ray_dir):
    v0,v1,v2 = _v(v0),_v(v1),_v(v2); o,d_vec = _v(ray_origin),_v(ray_dir)
    d_vec = d_vec / np.linalg.norm(d_vec)
    e1,e2 = v1-v0, v2-v0
    h = np.cross(d_vec, e2)
    a = np.dot(e1, h)
    if abs(a) < 1e-10: return {"hit": False, "reason": "射线平行于三角形平面"}
    f = 1.0 / a
    s = o - v0
    u = f * np.dot(s, h)
    if u < 0 or u > 1: return {"hit": False}
    q = np.cross(s, e1)
    v = f * np.dot(d_vec, q)
    if v < 0 or u + v > 1: return {"hit": False}
    t = f * np.dot(e2, q)
...
def op_triangle_triangle(v0,v1,v2, u0,u1,u2):
    ...
    v0,v1,v2 = _v(v0),_v(v1),_v(v2); u0,u1,u2 = _v(u0),_v(u1),_v(u2)
    def _project(tri, axis):
        dots = [np.dot(v, axis) for v in tri]; return min(dots), max(dots)
    tri1 = [v0, v1, v2]; tri2 = [u0, u1, u2]
    # 法向量作为分离轴
    n1 = np.cross(v1-v0, v2-v0); n2 = np.cross(u1-u0, u2-u0)
    # 边叉积作为分离轴
    axes = [n1, n2]
    ...
    for axis in axes:
        ...
        min1,max1 = _project(tri1, axis); min2,max2 = _project(tri2, axis)
        if max1 < min2 or max2 < min1: return {"intersect": False, "separating_axis": axis.tolist()}
...
def op_ellipse_contains_point(center, rx, ry, point, angle_deg=0):
    c = _v(center); p = _v(point); rx,ry = float(rx),float(ry)
    # 旋转到椭圆坐标系
    dp = p - c
...
def op_ellipse_ray(center, rx, ry, ray_origin, ray_dir, angle_deg=0):
    """射线与椭圆求交（2D）"""
    c = _v(center); o = _v(ray_origin); d_vec = _v(ray_dir)
    ...
    t2 = (-b + math.sqrt(disc)) / (2*a)
    pts = []
    if t1 >= 0: pts.append({"t": float(t1), "point": (o[:2] + t1*d_vec[:2]).tolist()})
    if t2 >= 0 and abs(t2-t1) > 1e-8: pts.append({"t": float(t2), "point": (o[:2] + t2*d_vec[:2]).tolist()})
...
def op_closest_point_on_segment(a, b, point):
    A,B,P = _v(a),_v(b),_v(point); closest, t = _closest_point_on_segment(A,B,P)
    return {"closest_point": closest.tolist(), "t": float(t),
            "distance": float(np.linalg.norm(P - closest))}
...
def op_segment_segment_distance(a1, b1, a2, b2):
    ...
    d00=np.dot(AB,AB); d11=np.dot(CD,CD); d01=np.dot(AB,CD)
    if d00 < 1e-15 and d11 < 1e-15: return {"distance": float(np.linalg.norm(C-A))}
    if d00 < 1e-15:
        s=0; t=np.clip(np.dot(AC,CD)/d11,0,1) if d11>1e-15 else 0
    elif d11 < 1e-15:
        s=np.clip(-np.dot(AC,AB)/d00,0,1); t=0
    else:
        det=d00*d11-d01*d01
        s_num=-d01*np.dot(AC,CD)+d11*np.dot(AC,AB)
        t_num=d00*np.dot(AC,CD)-d01*np.dot(AC,AB)
        if abs(det)<1e-15: s=t=0
        else:
            ...
            t=t_param
    p1=A+s*AB; p2=C+t*CD
    return {"distance": float(np.linalg.norm(p1-p2)), "closest_on_AB": p1.tolist(),
            "closest_on_CD": p2.tolist(), "s": float(s), "t": float(t)}
...
def op_point_to_plane(point, plane_point, plane_normal):
    ...
    dist = np.dot(p - pp, n)
    proj = p - dist * n
    return {"signed_distance": float(dist), "absolute_distance": float(abs(dist)),
            "projected_point": proj.tolist(), "side": "positive" if dist>0 else ("negative" if dist<0 else "on_plane")}
...
def main():
    if not HAS_NUMPY:
        print(json.dumps({"ok": False, "error": "需要 numpy"}, ensure_ascii=False)); sys.exit(1)
...
""")
    ...
    parser.add_argument('--center', help='球/椭圆中心'); parser.add_argument('--radius', type=float, help='半径')
    parser.add_argument('--c1','--c2', help='球体中心'); parser.add_argument('--r1','--r2', type=float, help='球体半径')
    ...
    parser.add_argument('--cap_a','--cap_b', help='胶囊端点'); parser.add_argument('--cap_r', type=float, help='胶囊半径')
    parser.add_argument('--sph_c', help='球心'); parser.add_argument('--sph_r', type=float, help='球半径')
    ...
    try:
        ...
        if op == 'sphere_contains_point':
            ...
            kwargs.update(ray_origin=_get('ray_origin'), ray_dir=_get('ray_dir','0,0,1'), center=_get('center','0,0,0'), radius=_get('radius',1))
        elif op in ('sphere_aabb',):
            kwargs.update(center=_get('center'), radius=_get('radius'), aabb_min=_get('aabb_min'), aabb_max=_get('aabb_max'))
            ...
            if op == 'capsule_sphere': kwargs.update(sph_c=_get('sph_c'), sph_r=_get('sph_r',1))
            elif op == 'capsule_capsule': kwargs.update(a2=_get('a2',_get('cap_a2')), b2=_get('b2',_get('cap_b2')), r2=_get('r2',_get('cap_r2',0.5)))
            elif op == 'capsule_aabb': kwargs.update(aabb_min=_get('aabb_min'), aabb_max=_get('aabb_max'))
            elif op == 'capsule_point': kwargs.update(point=_get('point'))
        elif op in ('aabb_contains_point',):
            kwargs.update(aabb_min=_get('aabb_min'), aabb_max=_get('aabb_max'), point=_get('point'))
        elif op == 'aabb_aabb':
            kwargs.update(a1_min=_get('aabb_min'), a1_max=_get('aabb_max'), a2_min=_get('a2_min',_get('aabb_min2')), a2_max=_get('a2_max',_get('aabb_max2')))
        elif op == 'ray_aabb':
            kwargs.update(ray_origin=_get('ray_origin'), ray_dir=_get('ray_dir'), aabb_min=_get('aabb_min'), aabb_max=_get('aabb_max'))
        elif op in ('triangle_point', 'triangle_ray', 'triangle_sphere', 'triangle_triangle'):
            kwargs.update(v0=_get('v0'), v1=_get('v1'), v2=_get('v2'))
            if op == 'triangle_point': kwargs.update(point=_get('point'))
            elif op == 'triangle_ray': kwargs.update(ray_origin=_get('ray_origin'), ray_dir=_get('ray_dir'))
            elif op == 'triangle_sphere': kwargs.update(center=_get('center'), radius=_get('radius'))
            elif op == 'triangle_triangle': kwargs.update(u0=_get('u0',_get('v3')), u1=_get('u1',_get('v4')), u2=_get('u2',_get('v5')))
        elif op in ('ellipse_contains_point', 'ellipse_ray'):
            kwargs.update(center=_get('center','0,0'), rx=_get('rx',1), ry=_get('ry',0.5), angle_deg=_get('angle',0))
            if op == 'ellipse_contains_point': kwargs.update(point=_get('point'))
            elif op == 'ellipse_ray': kwargs.update(ray_origin=_get('ray_origin'), ray_dir=_get('ray_dir'))
            ...
            kwargs.update(point=_get('point'), plane_point=_get('plane_point','0,0,0'), plane_normal=_get('plane_normal','0,0,1'))
