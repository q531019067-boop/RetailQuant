...
def _verify_numeric_detail(lhs_str, rhs_str, n):
    ...
    safe = {
        ...
        'exp': _m.exp, 'log': _m.log, 'sqrt': _m.sqrt,
        'pi': _m.pi, 'e': _m.e, 'abs': abs,
    }
    for _ in range(n):
        vars_dict = {}
...
def op_euler(expr_str=None, n_terms=10):
    ...
    for theta in test_angles:
        ...
        verifications.append({
            "theta": theta, "theta_pi": f"{theta/math.pi:.4g}pi",
    ...
    # 几何解释
    results['geometric'] = "欧拉公式 e^(iθ) = cos(θ) + i·sin(θ) 表明：复平面上，e^(iθ) 对应单位圆上角度为 θ 的点。当 θ=π 时，该点位于 (-1, 0)，因此 e^(iπ)+1=0 将五个最重要的数学常数 (e, i, π, 1, 0) 联系在一条等式中。"
...
def op_heron_area(a, b, c):
    ...
    return {"area": area, "semiperimeter": s, "sides": [a, b, c],
...
def op_law_of_sines(a=None, A=None, b=None, B=None, c=None, C=None):
    ...
    if len(given_angles) == 2:
        ...
        if A is None: result["angle_A"] = missing
        if B is None: result["angle_B"] = missing
        if C is None: result["angle_C"] = missing
    if result.get("diameter_2R"):
        d = result["diameter_2R"]
        if a is None and A is not None: result["side_a"] = d * math.sin(math.radians(A))
        if b is None and B is not None: result["side_b"] = d * math.sin(math.radians(B))
        if c is None and C is not None: result["side_c"] = d * math.sin(math.radians(C))
    return result
