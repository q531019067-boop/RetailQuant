...
MODIFY_PATTERNS = [
    # (正则匹配需要什么修改, 修改操作)
    (r'(平方|二次|2次)', lambda e: e.replace('**2', '**3').replace('** 2', '** 3') if '**2' in e else e.replace('*x', '**2*x').replace('x*x', 'x**3')),
    (r'(立方|三次|3次)', lambda e: e.replace('**2', '**3').replace('** 2', '** 3')),
    (r'(加.*衰减|乘以.*衰减|.*衰减因子)', lambda e: f'({e})*exp(-d*x)'),
    (r'(取反|翻折|镜像|相反数)', lambda e: f'-({e})'),
    (r'(平移|右移|左移).*?(\d+)', lambda e, m: f'({e.replace("x", f"(x-{m.group(2)})")})' if '右移' in m.group(1) else f'({e.replace("x", f"(x+{m.group(2)})")})'),
    (r'(放大|缩小|缩放).*?(\d+(?:\.\d+)?)倍', lambda e, m: f'{float(m.group(2))}*({e})'),
    (r'(向上|向下)平移\s*(\d+(?:\.\d+)?)', lambda e, m: f'({e})+{m.group(2)}' if '向上' in m.group(1) else f'({e})-{m.group(2)}'),
...
def op_compose(need_desc):
        ...
        for name, info in components.items():
            ...
            for prop in info['props']:
                for word in prop.split():
                    if word in need_desc: score += 2
            if name in need_desc.lower(): score += 5
            if score > 0:
                component_matches.append({
                    "category": category, "name": name,
                    "expr": info['expr'], "props": info['props'],
                    "relevance": score,
                })
    ...
    if component_matches:
        ...
        suggestions.append(f"### 推荐组件\n" + "\n".join(
            f"- **{c['name']}** ({c['category']}): `{c['expr']}` — {'; '.join(c['props'])}"
            for c in comps
        ))
    if not suggestions:
    ...
    for prop in properties:
        ...
        if '单调递增' in prop or 'monotonic_increasing' in prop_lower:
            ...
            if m:
                ...
                checks[f'值域在({lo},{hi})'] = {"passed": bool(in_range), "outliers": int(np.sum((ys < lo) | (ys > hi)))}
...
OPERATIONS = {
    'modify': op_modify, 'compose': op_compose,
    'verify': op_verify, 'components': op_components,
}
