...
def op_sample_adaptive(expr, var, range_, n_base=50, max_n=500):
    ...
    return {
        "points": all_points, "n": len(all_points), "range": [x_min, x_max],
        "y_min": float(min(all_ys)), "y_max": float(max(all_ys)),
        "adaptive": True, "base_n": coarse_n, "extra_n": int(extra_counts.sum()),
    }
...
def op_fit_poly(points, degree):
    if not HAS_NUMPY: return {"error": "需要 numpy"}
    ...
    return {
        ...
        "r_squared": float(r2), "degree": degree,
        "residual_std": float(np.sqrt(ss_res/len(xs))) if len(xs) > degree+1 else 0,
    }

def op_fit_linear(points): return op_fit_poly(points, 1)
...
def op_fit_exp(points):
    ...
    ys = np.array([p[1] for p in pts])
    valid = ys > 0
    if not np.all(valid): return {"error": "指数拟合需要 y > 0"}
    log_ys = np.log(ys)
...
def op_fit_log(points):
    ...
    ys = np.array([p[1] for p in pts])
    valid = xs > 0
    if not np.all(valid): return {"error": "对数拟合需要 x > 0"}
    log_xs = np.log(xs)
...
def op_fit_auto(points):
    pts = _to_points(points)
    models = {}
    for model_name, func in [('linear', op_fit_linear), ('poly2', lambda p: op_fit_poly(p, 2)),
                              ('poly3', lambda p: op_fit_poly(p, 3)), ('exp', op_fit_exp), ('log', op_fit_log)]:
        try: models[model_name] = func(pts)
...
def op_interp_linear(points, x_query):
    ...
    return {"interpolated": float(np.interp(x_query, xs, ys)), "at_x": x_query} if HAS_NUMPY else {"error": "需要 numpy"}
...
def op_interp_cubic(points, x_query=None, n_out=100):
    ...
    if x_query is not None: return {"interpolated": float(cs(x_query)), "at_x": x_query}
...
def _describe_structured(xs, ys):
    ...
    # 全局单调性
    if np.all(diffs >= -1e-10): desc['monotonicity'] = '全局单调递增'
    elif np.all(diffs <= 1e-10): desc['monotonicity'] = '全局单调递减'
    else: desc['monotonicity'] = f'非单调（{sum(1 for s in segments if "增" in s["type"])}个递增段，{sum(1 for s in segments if "减" in s["type"])}个递减段）'
    # 极值
    ...
    if n > 4:
        d2_total = np.sum(d2)
        if d2_total > 1e-8: desc['concavity'] = '整体凹向上（下凸）'
        elif d2_total < -1e-8: desc['concavity'] = '整体凹向下（上凸）'
        else: desc['concavity'] = '凹凸交替'
    # 对称性
    mid_x = (xs[0] + xs[-1]) / 2
    left = ys[xs < mid_x]
    right = ys[xs >= mid_x]
    if len(left) > 0 and len(right) > 0:
        ...
        if len(r_rev) > 2:
            ...
            desc['symmetry'] = {'error': sym_err, 'likely_symmetric': sym_err < 0.05 * (desc['range'][1] - desc['range'][0] + 1e-10)}
    # 统计
...
def _narrative_global(xs, ys, desc):
    ...
    parts.append(f"于 x={gmax['x']:.4g} 处取得全局最大值 {gmax['y']:.4g}，于 x={gmin['x']:.4g} 处取得全局最小值 {gmin['y']:.4g}。")
    # 凹凸
...
def _narrative_induction(xs, ys, desc, n_samples):
    ...
    if len(segments) == 1:
        ...
        parts.append(f"函数在采样区间内经历 {len(segments)} 次单调性转变，推测连续函数至少存在 {len(segments)-1} 个临界点。")
    # 平滑性推断
    ...
    if local:
        parts.append(f"局部极值点的分布模式暗示函数可能存在 {len(local)} 个驻点（导数为零点），其位置可通过牛顿法或二分法精确求解。")
    return "".join(parts)
...
def op_multi_scale(expr, var, range_, scales=None):
    ...
    if len(results) >= 2:
        comparison = {
            "stability": "各尺度下均值与值域高度一致" if all(
                abs(r['mean'] - results[0]['mean']) < 0.1 * abs(results[0]['mean']) + 1e-6
                for r in results[1:]
...
def op_describe(points_or_expr, var='x', range_=None, mode='structured'):
    ...
    if isinstance(points_or_expr, str) and not points_or_expr.startswith('[['):
        if not range_: range_ = [-10, 10]
        sample_r = op_sample(points_or_expr, var, range_, 200)
        if 'error' in sample_r: return sample_r
        points = sample_r['points']
    else:
...
OPERATIONS = {
    ...
    'fit_poly': op_fit_poly, 'fit_linear': op_fit_linear,
    'fit_exp': op_fit_exp, 'fit_log': op_fit_log, 'fit_auto': op_fit_auto,
    'interp_linear': op_interp_linear, 'interp_cubic': op_interp_cubic,
    'bezier_eval': op_bezier_eval,
    'describe': op_describe, 'narrate': op_narrate, 'multi_scale': op_multi_scale,
}
  ...
  python curve_tools.py --op "describe" --expr "sin(x)" --mode "narrative"
    ...
    try:
        ...
        if op in ('sample', 'sample_adaptive'):
            ...
            if op == 'fit_poly': kwargs['degree'] = args.degree or input_data.get('degree', 2)
        elif op.startswith('interp'):
            kwargs['points'] = args.points or input_data.get('points', [])
            if args.x_query is not None: kwargs['x_query'] = args.x_query
            elif 'x_query' in input_data: kwargs['x_query'] = input_data['x_query']
        elif op == 'bezier_eval':
            kwargs['controls'] = args.controls or input_data.get('controls', [])
            kwargs['n'] = args.n or input_data.get('n', 50)
            if args.t is not None: kwargs['t'] = args.t
            elif 't' in input_data: kwargs['t'] = input_data['t']
        elif op in ('describe', 'narrate', 'multi_scale'):
            expr = args.expr or input_data.get('expr')
            points = args.points or input_data.get('points')
            if points: kwargs['points_or_expr'] = points
            elif expr: kwargs['points_or_expr'] = expr
            kwargs['var'] = args.var or input_data.get('var', 'x')
        ...
        if len(out_str) > 100000:
            for key in ['points', 'narratives']:
                if key in output: output[key] = f"[truncated]"
            out_str = json.dumps(output, ensure_ascii=False, indent=None if args.compact else 2, default=str)
        print(out_str)
    except Exception as e:
