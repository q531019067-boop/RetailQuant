...
def _parse_complex(s):
    ...
    if isinstance(s, (list, tuple)) and len(s) >= 2: return complex(s[0], s[1])
    if isinstance(s, dict): return complex(s.get('re', 0), s.get('im', 0))
    if isinstance(s, str):
        s = s.strip()
...
def _to_json(z):
    """复数→JSON友好输出"""
    return {"re": z.real, "im": z.imag, "string": f"{z.real:.6g}{z.imag:+6g}j",
            "modulus": abs(z), "argument_rad": cmath.phase(z), "argument_deg": math.degrees(cmath.phase(z))}
...
def _geometric_multiply(z1, z2):
    ...
    return (f"复数乘法的几何意义：模长相乘（{r1:.4g}×{r2:.4g}={r1*r2:.4g}），"
...
def _geometric_divide(z1, z2):
    ...
    r1, t1 = abs(z1), cmath.phase(z1)
    r2, t2 = abs(z2), cmath.phase(z2)
    return (f"复数除法的几何意义：模长相除（{r1:.4g}÷{r2:.4g}={r1/r2:.4g}），"
...
def op_arithmetic(a_str, b_str, func):
    """基本运算"""
    a = _parse_complex(a_str); b = _parse_complex(b_str)
    ops = {
        'add': ('+', a + b, "加法对应复平面上向量的平行四边形法则"),
        'sub': ('-', a - b, "减法等于加上相反数，几何上对应向量差"),
        'mul': ('×', a * b, _geometric_multiply(a, b)),
        'div': ('÷', a / b if b != 0 else complex(float('nan')),
                _geometric_divide(a, b) if b != 0 else "除数不能为零"),
    }
    if func not in ops:
        return {"error": f"不支持: {func}，可用: add/sub/mul/div"}
    op_symbol, result, geometric = ops[func]
    return {
        "op": func, "symbol": op_symbol,
        "a": _to_json(a), "b": _to_json(b),
...
def op_modulus(a_str):
    ...
    r = abs(z)
    return {"z": _to_json(z), "modulus": r, "modulus_squared": r*r,
...
def op_argument(a_str):
    ...
    for num, den in [(1,6),(1,4),(1,3),(1,2),(2,3),(3,4),(5,6),(1,1)]:
        if abs(pi_ratio - num/den) < 0.001:
            ...
            break
    if not pi_str and abs(pi_ratio + 1) < 0.001: pi_str = " = -π"
    return {"z": _to_json(z), "argument_rad": arg_rad, "argument_deg": arg_deg,
...
def op_conjugate(a_str):
    z = _parse_complex(a_str)
    conj = z.conjugate()
    return {"z": _to_json(z), "conjugate": _to_json(conj),
...
def op_rect_to_polar(a, b):
    ...
    return {"rectangular": {"x": a, "y": b, "string": f"{a}{b:+}j"},
            "polar": {"r": r, "theta_rad": theta, "theta_deg": math.degrees(theta),
...
def op_polar_to_rect(r, theta_deg):
    ...
    y = r * math.sin(theta)
    z = complex(x, y)
    return {"polar": {"r": r, "theta_deg": theta_deg},
...
def op_rotate(a, b, angle_deg):
    ...
    z_rot = z * rot
    return {"original": _to_json(z), "rotation_angle_deg": angle_deg,
...
def op_de_moivre(r, theta_deg, n):
    ...
    return {
        ...
        "n": n,
        "result_polar": {"r": r_n, "theta_deg": math.degrees(theta_n) % 360,
        ...
        "error": float(err),
        "geometric": (f"棣莫弗定理：将复数自乘 {n} 次，模长变为原来的 {n} 次方（{r}^{n}={r_n:.4g}），"
...
def op_roots(a, b, n):
    ...
    return {
        "z": _to_json(z), "n": n,
        "roots": roots,
        "geometric": (f"复数 {z.real:.4g}{z.imag:+4g}j 的 {n} 次方根均匀分布在半径为 {r**(1/n):.4g} 的圆上，"
...
def op_visualize(a, b):
    ...
    quadrant = 1 if z.real>=0 and z.imag>=0 else (2 if z.real<0 and z.imag>=0 else (3 if z.real<0 and z.imag<0 else 4))
    quad_names = {1: "第一象限（右上）", 2: "第二象限（左上）", 3: "第三象限（左下）", 4: "第四象限（右下）"}

    # 与单位圆的关系
    unit_circle = "在单位圆上" if abs(r-1) < 0.001 else (f"在单位圆{'外' if r>1 else '内'}（距离圆心 {abs(r-1):.4g}）")

    # 特殊角检测
    special_angles = {0: "正实轴", 90: "正虚轴", 180: "负实轴", 270: "负虚轴",
                      45: "一三象限对角线", 135: "二四象限对角线"}
    angle_desc = ""
