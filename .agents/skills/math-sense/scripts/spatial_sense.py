...
def _parse_vec(s):
    if isinstance(s, (list, tuple)): return [float(v) for v in s[:3]]
    if isinstance(s, str):
        s = s.strip()
        if s.startswith('['): return [float(v) for v in json.loads(s)[:3]]
...
BEARING_NAMES = [
    (0, "正北"), (22.5, "北东北"), (45, "东北"), (67.5, "东东北"),
    (90, "正东"), (112.5, "东东南"), (135, "东南"), (157.5, "南东南"),
    (180, "正南"), (202.5, "南西南"), (225, "西南"), (247.5, "西西南"),
    (270, "正西"), (292.5, "西西北"), (315, "西北"), (337.5, "北西北"),
    (360, "正北"),
]

BEARING_WORDS_PATTERNS = [
    (r'正北', 0), (r'北东北|北偏东', 22.5), (r'东北', 45), (r'东东北|东偏北', 67.5),
    (r'正东', 90), (r'东东南|东偏南', 112.5), (r'东南', 135), (r'南东南|南偏东', 157.5),
    (r'正南', 180), (r'南西南|南偏西', 202.5), (r'西南', 225), (r'西西南|西偏南', 247.5),
    (r'正西', 270), (r'西西北|西偏北', 292.5), (r'西北', 315), (r'北西北|北偏西', 337.5),
]
...
def op_bearing_to_words(angle_deg):
    ...
    return {
        "angle_deg": angle_deg, "angle_rad": math.radians(angle_deg),
    ...
    for pattern, base_angle in BEARING_WORDS_PATTERNS:
        if re.search(pattern, desc):
            ...
            if om:
                ...
                if direction in ('西', '北'): offset = -offset
            angle = (base_angle + offset) % 360
            return {"description": desc, "angle_deg": angle, "base": base_angle,
                    "offset": offset, "confidence": "approximate"}

    return {"description": desc, "angle_deg": None, "confidence": "unknown",
...
def op_describe_to_math(description):
    ...
    # 方向词 → 向量
    dir_map = {'前': (0, 1, 0), '后': (0, -1, 0), '左': (-1, 0, 0), '右': (1, 0, 0),
               '上': (0, 0, 1), '下': (0, 0, -1),
               '前方': (0, 1, 0), '后方': (0, -1, 0), '左方': (-1, 0, 0), '右方': (1, 0, 0),
               '上方': (0, 0, 1), '下方': (0, 0, -1)}
    dir_vector = None
...
def op_convert_point(point, from_hand, to_hand, flip='z'):
    ...
    return {"original": p.tolist(), "converted": result.tolist(),
            "from": from_hand, "to": to_hand, "flipped_axis": flip,
...
def op_convert_rotation(rotation, from_hand, to_hand, format='quat', flip='z'):
    ...
    if not HAS_NUMPY: return {"error": "需要 numpy"}
    if from_hand == to_hand: return {"rotation": rotation, "note": "同手系无需转换"}
    if format == 'quat':
        q = np.array(_parse_vec(rotation), dtype=float)
        if len(q) != 4: return {"error": "四元数需4分量 [w,x,y,z]"}
        q = q / np.linalg.norm(q)
        w, x, y, z = q
        if flip == 'z': qn = np.array([w, -x, -y, z])
        elif flip == 'y': qn = np.array([w, -x, y, -z])
        else: qn = q
        return {"original_quat": q.tolist(), "converted_quat": (qn/np.linalg.norm(qn)).tolist(),
                "note": f"翻转{flip}轴→取反相关分量。旋转角度不变，方向可能反转"}
    elif format == 'euler':
        r = rotation if isinstance(rotation,(list,tuple)) else _parse_vec(rotation)
        yaw, pitch, roll = r[0], r[1], r[2]
        if flip == 'z': c = [-yaw, pitch, -roll]
        elif flip == 'y': c = [yaw, -pitch, roll]
        else: c = list(r)
...
def op_convert_matrix(matrix, from_hand, to_hand, flip='z'):
    ...
    if M.shape != (4,4) and len(M.shape) == 2:
        M = M[:4,:4]
    if from_hand == to_hand: return {"matrix": M.tolist()}
    F = np.eye(4)
    if flip == 'z': F[2,2] = -1
    elif flip == 'y': F[1,1] = -1
    M_new = F @ M @ F
    return {"original_matrix": M.tolist(), "converted_matrix": M_new.tolist(),
            "note": f"翻转{flip}轴: F·M·F, F=diag(1,{'-1' if flip=='y' else '1'},{'-1' if flip=='z' else '1'},1)"}

def op_list_handedness():
    """列出常见引擎的坐标系约定"""
    return {"engines": HANDEDNESS,
