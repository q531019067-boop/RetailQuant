...
def _proportion_phrase(ratio):
    """将比例转为自然语言"""
    if ratio <= 0: return "为零"
    if ratio >= 1: return "为全部 (100%)"
...
def op_proportion(ratio):
    ...
    context = ""
    if ratio < 0.001:
        context = "在统计学中，这个概率可以被视为'几乎不可能发生'。如果每天做一次，平均需要约 {:.0f} 年才会遇到一次。".format(1/(ratio*365) if ratio > 0 else float('inf'))
    elif ratio < 0.01:
...
def op_convert(value, from_unit, to_unit):
    ...
    return {"error": f"不支持的转换: {from_unit} → {to_unit}。支持: fps↔frame_ms, m↔km, s↔ms↔min↔h, kg↔g, GB↔MB, rad↔deg, percent↔ratio"}
...
OPERATIONS = {
    'order': op_order, 'compare': op_compare, 'proportion': op_proportion,
    'analogy': op_analogy, 'convert': op_convert, 'anchors': op_anchors,
}
