...
def op_is_prime(n):
    ...
    n = int(n)
    if n < 2:
        return {"n": n, "is_prime": False, "reason": "小于2"}
    if n in (2, 3):
        return {"n": n, "is_prime": True}
    if n % 2 == 0 or n % 3 == 0:
        return {"n": n, "is_prime": False, "reason": f"可被{'2' if n % 2 == 0 else '3'}整除"}
    i = 5
    while i * i <= n:
        if n % i == 0 or n % (i + 2) == 0:
...
def op_miller_rabin(n, k=10):
    ...
    if n in (2, 3):
        return {"n": n, "is_prime": True, "method": "trivial"}
    if n % 2 == 0:
        return {"n": n, "is_prime": False, "method": "even"}
    ...
    return {"n": n, "is_prime": "probably", "method": "Miller-Rabin", "k": k, "error_bound": f"< {4 ** (-k):.2e}"}
...
def op_sieve(limit):
    ...
    limit = int(limit)
    if limit < 2:
        return {"primes": [], "count": 0, "limit": limit}
...
def op_factor(n):
    ...
    exponent_map = {}
    for f in factors:
        exponent_map[str(f)] = exponent_map.get(str(f), 0) + 1
    latex_str = " \\cdot ".join(
        f"{p}^{{{e}}}" if e > 1 else str(p) for p, e in sorted([(int(k), v) for k, v in exponent_map.items()])
    )
    return {
        "n": n_orig,
        "factors": factors,
...
def op_totient(n):
    ...
    for f in result["factors"]:
        if f not in seen:
            phi = phi * (f - 1) // f
            seen.add(f)
    return {"n": n, "totient": phi, "formula": f"φ({n}) = {n} × " + " × ".join(f"(1-1/{p})" for p in seen)}
...
def op_prime_gaps(limit=1000):
    ...
    return {
        ...
        "max_gap": max_gap,
        "max_gap_at": primes[gaps.index(max_gap) + 1] if gaps else None,
...
if __name__ == "__main__":
    ...
    if "n" in sig and args.n is not None:
        kwargs["n"] = args.n
    if "k" in sig and args.k is not None:
        kwargs["k"] = args.k
    if "a" in sig and args.a is not None:
        kwargs["a"] = args.a
    if "b" in sig and args.b is not None:
        kwargs["b"] = args.b
    if "limit" in sig:
