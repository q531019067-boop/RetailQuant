...
def sympy_diff(expr_str, var_str, order=1, point=None):
    x = sp.Symbol(var_str); expr = sp.sympify(expr_str)
    deriv = sp.diff(expr, x, order)
    result = {"symbolic": str(deriv), "latex": sp.latex(deriv)}
    if point is not None:
        val = float(deriv.subs(x, point)); result["at_point"] = point; result["numeric"] = val
        try:
            f = sp.lambdify(x, expr, 'numpy'); h = 1e-8
...
def sympy_integrate(expr_str, var_str, a=None, b=None):
    ...
    if a is not None and b is not None:
        ...
        result = {"definite": float(sp.integrate(expr, (x, a, b)).evalf()),
                  "indefinite": str(indefinite), "latex_indefinite": sp.latex(indefinite), "a": a, "b": b}
        if HAS_SCIPY:
            try:
                f = sp.lambdify(x, expr, 'numpy'); num_val, _ = integrate_sp.quad(f, a, b)
...
def sympy_root(expr_str, var_str, guess=None, interval=None):
    ...
    try: result["symbolic_roots"] = [str(r) for r in sp.solve(expr, x)]
    except Exception: pass
    if HAS_SCIPY:
        f = sp.lambdify(x, expr, 'numpy')
        if guess is not None:
            try: root = opt.newton(f, guess); result["newton_root"] = float(root); result["f_at_root"] = float(f(root))
            except Exception as e: result["newton_error"] = str(e)
        elif interval and len(interval) == 2:
            try: root = opt.brentq(f, interval[0], interval[1]); result["brent_root"] = float(root); result["f_at_root"] = float(f(root))
            except Exception as e: result["brent_error"] = str(e)
    return result
...
def sympy_solve(equations, variables):
    ...
    try:
        ...
        result = {"solutions": [{str(k): str(v) for k, v in s.items()} for s in sol],
                  "variables": variables, "count": len(sol)}
...
def sympy_dsolve(ode_str, func_str="y", var_str="x"):
    ...
    x = sp.Symbol(var_str); y = sp.Function(func_str)(x)
    try:
...
def sympy_ode_numeric(ode_expr, func_str, var_str, y0, t_span, t_eval=None):
    ...
    if not HAS_SCIPY: return {"error": "需要 scipy"}
    import numpy as np
...
def sympy_eval(expr_str, vars_dict=None, points=None):
    ...
    if points:
        ...
        for pt in points:
            subs = {}
            if isinstance(pt, dict): subs = {sp.Symbol(k): v for k, v in pt.items()}
            elif isinstance(pt, (list, tuple)):
                free_vars = list(expr.free_symbols); subs = {v: pt[i] for i, v in enumerate(free_vars) if i < len(pt)}
...
def sympy_partial_diff(expr_str, vars_list, order=1, point=None):
    """偏导数"""
    symbols = [sp.Symbol(v) for v in vars_list]; expr = sp.sympify(expr_str)
    results = {}
    ...
    if point and len(point) == len(vars_list):
        ...
        out["at_point"] = point
        out["values"] = {f"d/d{var_str}": float(sp.diff(expr, sym, order).subs(subs).evalf())
                         for var_str, sym in zip(vars_list, symbols)}
    return out
...
def sympy_gradient(expr_str, vars_list, point=None):
    ...
    result = {"gradient": [str(g) for g in grad_expr],
              "gradient_latex": sp.latex(sp.Matrix(grad_expr)), "variables": vars_list}
    if point and len(point) == len(vars_list):
        subs = {s: point[i] for i, s in enumerate(symbols)}
        gv = [float(g.subs(subs).evalf()) for g in grad_expr]
        result["at_point"] = point; result["gradient_value"] = gv
        result["magnitude"] = float(sum(v**2 for v in gv) ** 0.5)
        result["geometric"] = f"梯度向量指向函数值增长最快的方向，模长为 {result['magnitude']:.4g}。在点{point}处，最陡上升方向为 {gv}。"
    return result
...
def sympy_hessian(expr_str, vars_list, point=None):
    ...
    result = {"hessian": [[str(hess[i][j]) for j in range(n)] for i in range(n)],
              "hessian_latex": sp.latex(sp.Matrix(hess)), "variables": vars_list}
    if point and len(point) == n:
        subs = {s: point[i] for i, s in enumerate(symbols)}
        hnum = [[float(hess[i][j].subs(subs).evalf()) for j in range(n)] for i in range(n)]
        result["at_point"] = point; result["hessian_numeric"] = hnum
        if HAS_NUMPY:
            ...
            if all_pos: result["critical_type"] = "局部极小值点（Hessian正定）"
            elif all_neg: result["critical_type"] = "局部极大值点（Hessian负定）"
            elif mixed: result["critical_type"] = "鞍点（Hessian不定）"
            else: result["critical_type"] = "无法确定（Hessian半定）"
            result["geometric"] = f"Hessian特征值: {[float(f'{v.real:.4g}') for v in eigvals]} → {result['critical_type']}"
    return result


def sympy_jacobian(exprs_list, vars_list, point=None):
    """Jacobian 矩阵"""
    symbols = [sp.Symbol(v) for v in vars_list]; exprs = [sp.sympify(e) for e in exprs_list]
    m, n = len(exprs), len(symbols)
    jac = [[sp.diff(exprs[i], symbols[j]) for j in range(n)] for i in range(m)]
    result = {"jacobian": [[str(jac[i][j]) for j in range(n)] for i in range(m)],
              "jacobian_latex": sp.latex(sp.Matrix(jac)), "functions": exprs_list, "variables": vars_list}
    if point and len(point) == n:
        subs = {s: point[i] for i, s in enumerate(symbols)}
        jnum = [[float(jac[i][j].subs(subs).evalf()) for j in range(n)] for i in range(m)]
        result["at_point"] = point; result["jacobian_numeric"] = jnum
...
def sympy_divergence(exprs_list, vars_list, point=None):
    ...
    if point and len(point) == len(vars_list):
        subs = {s: point[i] for i, s in enumerate(symbols)}
        result["at_point"] = point; result["value"] = float(div.subs(subs).evalf())
        result["geometric"] = "散度度量向量场在某点'源'或'汇'的强度。正值→向外发散(源)，负值→向内汇聚(汇)，零→无源无汇。"
    return result
...
def sympy_numeric_integrate(expr_str, var_str, a, b, n=1000):
    ...
    x = sp.Symbol(var_str); expr = sp.sympify(expr_str)
    f = sp.lambdify(x, expr, 'numpy')
    xs = np.linspace(a, b, n); ys = f(xs)
    trapz = float(np.trapz(ys, xs))
    dx = (b - a) / (n - 1)
    if n % 2 == 0: n_s = n - 1
    else: n_s = n
    xs_s = np.linspace(a, b, n_s); ys_s = f(xs_s)
    if n_s > 2:
...
def sympy_curve_length(expr_str, var_str, a, b, n=1000):
    ...
    xs = np.linspace(a, b, n); ys = f_int(xs)
...
def sympy_curvature(expr_str, var_str, at_point=None):
    x = sp.Symbol(var_str); expr = sp.sympify(expr_str)
    f1 = sp.diff(expr, x); f2 = sp.diff(f1, x)
    ...
    if at_point is not None:
        ...
        result["at_point"] = at_point; result["curvature"] = val
...
def sympy_area_between(expr1_str, expr2_str, var_str, a, b):
    import numpy as np
    x = sp.Symbol(var_str); f = sp.sympify(expr1_str); g = sp.sympify(expr2_str)
    diff = sp.Abs(f - g)
    try:
    ...
    result["between"] = [expr1_str, expr2_str]; result["a"] = a; result["b"] = b
    result["geometric"] = f"曲线 {expr1_str} 和 {expr2_str} 在[{a},{b}]间面积≈{result['area']:.6g}。"
    return result


OPERATIONS = {
    'diff': sympy_diff, 'integrate': sympy_integrate, 'limit': sympy_limit,
    'root': sympy_root, 'solve': sympy_solve,
    'linsolve': sympy_linsolve,
    'simplify': lambda expr, **kw: sympy_simplify(expr),
    'expand': lambda expr, **kw: sympy_expand(expr),
    'taylor': sympy_taylor, 'eval': sympy_eval,
    'partial_diff': sympy_partial_diff, 'gradient': sympy_gradient,
    'hessian': sympy_hessian, 'jacobian': sympy_jacobian, 'divergence': sympy_divergence,
    'integrate_numeric': sympy_numeric_integrate, 'curve_length': sympy_curve_length,
    'curvature': sympy_curvature, 'area_between': sympy_area_between,
}
...
""")
    ...
    input_data = {}
    if args.json_input:
        try: input_data = json.loads(args.json_input)
        except json.JSONDecodeError:
            print(json.dumps({"ok": False, "error": "JSON 解析失败"}, ensure_ascii=False)); sys.exit(1)
    elif not sys.stdin.isatty():
        raw = sys.stdin.read().strip()
        if raw:
            try: input_data = json.loads(raw)
            except json.JSONDecodeError:
                print(json.dumps({"ok": False, "error": "stdin JSON 解析失败"}, ensure_ascii=False)); sys.exit(1)
    ...
    if op not in ('jacobian', 'divergence') and not expr:
        print(json.dumps({"ok": False, "error": "缺少 expr"}, ensure_ascii=False)); sys.exit(1)
    if not op:
        print(json.dumps({"ok": False, "error": "缺少 op"}, ensure_ascii=False)); sys.exit(1)
    ...
    try:
        ...
        if op in ('diff',):
            ...
            point = args.point or input_data.get('point'); kwargs['point'] = point
        elif op == 'integrate':
            ...
            if args.a is not None and args.b is not None: kwargs['interval'] = [args.a, args.b]
            else: kwargs['interval'] = input_data.get('interval')
        elif op == 'taylor':
            kwargs['point'] = args.point if args.point is not None else input_data.get('point', 0)
            ...
            if op in ('jacobian', 'divergence'):
                ...
                kwargs['exprs_list'] = json.loads(el) if isinstance(el, str) and el.startswith('[') else input_data.get('exprs', [el])
