...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "formula_desc", __file__.replace("formula_explain.py", "formula_desc.py")
    )
...
def op_analyze(expr_str):
    ...
    if hints_count == 0:
        ...
        analysis["overall"] = (
            "该公式较复杂，建议：①先讲动机（为什么需要这个公式）②拆解为3-5个子部分分别讲③用具体数值代入演示④最后合成完整形式。"
        )
...
def op_explain(expr_str, audience="general"):
    ...
    if analysis.get("variable_count", 0) > 0:
        summary_parts.append(
            f"涉及{analysis['variable_count']}个变量：{', '.join(analysis.get('free_variables', []))}。"
        )
