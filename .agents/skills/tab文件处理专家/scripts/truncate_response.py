        ...
        if field in new_response and isinstance(new_response[field], list):
            ...
            for reduce in range(1, original_len + 1):
                ...
                if len(json.dumps(new_response, ensure_ascii=False)) <= MAX_OUTPUT_CHARS:
                    ...
                    hint += "\n2. 使用start_index分页获取更多数据（当前返回" + str(len(truncated)) + "条，下次从" + str(len(truncated)) + "开始）"
                    if column_truncated:
                    ...
                    new_response['hint'] = {
                        ...
                        "suggestion": "使用return_columns指定列，或使用start_index分页" + ("，或使用get_row_columns处理多列" if column_truncated else "，或使用read_rows处理多行")
                    }
                    return new_response
