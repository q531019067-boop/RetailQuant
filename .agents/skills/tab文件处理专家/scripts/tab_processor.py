...
class TabTable:
    ...
    def search(self, keyword: str = None, columns: Optional[List[str]] = None, limit: int = 100, start_index: int = 0, 
               return_columns: Optional[List[str]] = None, is_regex: bool = False, whole_word: bool = False,
               case_sensitive: bool = False, conditions: Optional[List[Dict[str, Any]]] = None, 
               logic: str = "and", count_only: bool = False) -> Tuple[List[Dict[str, str]], List[int], int]:
    ...
    def filter_rows(self, row_indices: List[int], column_names: List[str], start_index: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    def get_row_columns(self, row_index: int, column_names: Optional[List[str]] = None, 
                       column_conditions: Optional[List[Dict[str, Any]]] = None, logic: str = "or",
                       column_ranges: Optional[List[Dict[str, int]]] = None, 
                       column_indices: Optional[List[int]] = None, start_index: int = 0, 
                       limit: int = 50, skip_empty: bool = False, 
                       empty_values: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for col in paginated_columns:
            columns_data.append({
                'column_name': col,
                'column_index': self.column_names.index(col),
                'value': row.get(col, '')
            })
...
PREFERRED_SLIM_COLUMNS = [
    'ID', 'Name', 'Title', 'DisplayName', 'Model', 'Kind', 'Level', 'MapName',
]
...
def search_auto_slim_env() -> Tuple[bool, int, List[str], int]:
    ...
    threshold = _env_int_clamped(
        'TAB_PROCESSOR_WIDE_MIN_COLUMNS', WIDE_TABLE_MIN_COLUMNS, 1, 50000,
    )
...
def format_search_rows_tsv(rows: List[Dict[str, str]]) -> str:
    ...
    lines = ['\t'.join(cols)]
    for row in rows:
        lines.append('\t'.join(str(row.get(c, '')).replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for c in cols))
    return '\n'.join(lines) + '\n'
...
def process_request(request):
    ...
    try:
        ...
        if action == 'get_table_info':
            ...
            if (
                ...
                effective_return_columns = auto_slim_return_columns(
                    table, cond_for_slim, columns, slim_preferred, slim_fallback,
                )
                auto_slimmed = True

            matches, lines, total = table.search(
                keyword, columns, limit, start_index, effective_return_columns,
                is_regex, whole_word, case_sensitive, conditions, logic, count_only
            )
            ...
            if count_only:
                ...
                if (
                    not user_return_columns
                    and not auto_slimmed
                    and len(matches) > 0
                    and len(table.column_names) > 10
                ):
                    hints.append({
            ...
            result = {
                "message": f"行已添加，新行号: {table.total_rows - 1}"
            }
            ...
            if row_index is None or values is None:
                return {
                    "success": False,
                    "error": "缺少必要参数: row_index和values"
                }
            table.insert_row(row_index, values)
            result = {
                "message": f"行已插入到位置: {row_index}"
            }
            ...
            if not column_name:
                return {
                    "success": False,
                    "error": "缺少必要参数: column_name"
                }
            table.add_column(column_name, default_value, position)
            result = {
                "message": f"列'{column_name}'已添加"
            }
            ...
            if not old_name or not new_name:
                return {
                    "success": False,
                    "error": "缺少必要参数: old_name和new_name"
                }
            table.rename_column(old_name, new_name)
            result = {
                "message": f"列'{old_name}'已重命名为'{new_name}'"
            }
            ...
            result = table.get_row_columns(
                row_index, column_names, column_conditions, logic,
                column_ranges, column_indices, start_index, limit,
                skip_empty, empty_values
            )
        ...
        return {
            "success": False,
            "error": str(e),
            "hint": "如果问题持续存在，请检查文件格式是否正确，或联系技术支持"
        }
...
if __name__ == "__main__":
    ...
    def print_usage():
        ...
        print()
        print("多条件搜索（新功能）:")
        print("  python tab_processor.py --action search --file xxx.tab --column MaxPlayerCount=25 --column Type=1 --logic and")
        print("  python tab_processor.py --action search --file xxx.tab --conditions '[{\"columns\":[\"MaxPlayerCount\"],\"keyword\":\"25\"},{\"columns\":[\"Type\"],\"keyword\":\"1\"}]' --logic and")
        print()
        print("统计功能（新功能）:")
        print("  python tab_processor.py --action search --file xxx.tab --column MaxPlayerCount=25 --column Type=1 --count_only true")
        print()
        print("支持的操作:")
        ...
        print("实用示例:")
        print("  # 查询25人副本数量")
        print("  python tab_processor.py --action search --file MapList.tab --column MaxPlayerCount=25 --column Type=1 --count_only true")
        print()
        print("  # 获取25人副本的详细信息")
        print("  python tab_processor.py --action search --file MapList.tab --column MaxPlayerCount=25 --column Type=1 --return_columns ID,Name,DisplayName")
        print()
        print("  # 查看文件结构")
        ...
        while i < len(args):
            if args[i].startswith('--'):
                ...
                if i + 1 < len(args) and not args[i + 1].startswith('--'):
                    value = args[i + 1]
                    # 某些参数应该保持字符串类型
                    string_params = ['keyword', 'column', 'columns', 'column_names', 'return_columns', 
                                    'old_name', 'new_name', 'column_conditions', 'file', 'filepath', 'conditions', 'logic',
                                    'output_encoding', 'output_format']
