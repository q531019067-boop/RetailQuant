...
> 更新：2026-06-22 | 面向 AI 维护者 | 当前以 `rquant/` 包化架构为准
>
> 最近变更：删除根目录 `strategy.py` 与 `rquant/compat/`（策略统一走 `rquant.strategy`）；3 处 P0 修复详见 `docs/CHANGELOG.md`
...
- **场景样例轨**（`examples/`）：可复现的研究样本，薄编排组合 `scripts/` 层命令；报告与 CSV/JSON 产物与样例同目录存放，不写入 `docs/` 根层。
...
| `docs/STRATEGIES.md` | 12 个策略的触发条件与参数 |
...
> 2026-06-23 快照 | 以实际文件树为准；`cache/`、`results/`、`data/parquet/` 等为本地生成，默认不入 Git
...
flowchart TB
  subgraph entry["入口与配置"]
    ...
    cfg["config.toml / pyproject.toml"]
  end
...
├── config.toml                     # 运行时配置（端口、缓存 TTL、路径等）
...
│   └── log/                        #   loguru 封装（mq / web 等共用）
│
├── scripts/                        # CLI 研究 / 数据准备（调用 rquant/）
│   ├── run.py                      #   可选启动入口（同 app.py）
...
│       ├── run.py                  #   10 股 × 10 策略，组合 compare_strategies
│       ├── report.md               #   样例报告
│       └── results/                #   CSV / JSON 产物（本地）
│
├── templates/                      # Flask 模板
│   ├── index.html                  #   单页看板
...
│   ├── 大纲.md                     #   本文件：架构总纲
│   ├── 代码索引.md / 中英对照表.md
│   ├── STRATEGIES.md / CHANGELOG.md / TODOLIST.md
...
| `examples/` | 可复现场景样例，组合 scripts/；报告与 results 就近存放 | 跟踪（results 子目录可忽略） |
...
| `rquant/strategy/legacy/` | `ChanLun2B` / `BuyHold`（与其他策略一样 `@register`） |
| `rquant/strategy/factor/` | 多因子策略 `MultiFactor`（8 个 K 线技术因子）；历史财务因子代码在 `factor_calc.py` |

**策略入口（当前）：** 全部从 `rquant.strategy` 导入，无根目录 `strategy.py`：

```python
from rquant.strategy import scan_stock, scan_sell, scan_category, get, all_strategies
from rquant.strategy.legacy.chanlun2b import ChanLun2B   # 需单策略类时
```

Web / CLI / 研究链路均直接调用上述 API；`chanlun2b_signal()` 等老函数名已移除。
...
| `app.py` | 根目录 Web 启动入口（`create_app` / `run`） | 通常不改 |
...
| 调用策略扫描 | `from rquant.strategy import scan_stock, scan_sell` | 勿使用已删除的 `strategy.py` / `rquant.compat` |
...
1. **策略逻辑只在 `rquant/strategy/`**；根目录 `app.py` 仅启动 Web，不再存在 `strategy.py` / `rquant/compat/`。
